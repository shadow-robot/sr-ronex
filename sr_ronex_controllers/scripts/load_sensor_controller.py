...
class LoadSensorControllers(object):
    """
    Load the sensor controllers for all the RoNeXes present on the bus.
    """
    ...
    def find_ronexes(self):
        ...
        if attempts > 0:
            ...
            for key in ronex_param:
                if ronex_param[key]['product_name'] == 'spi':
    ...
    def set_param(self, ronex_ids):
        ...
        for ronex_id in ronex_ids:
            if ronex_id[1] == "spi":
                rospy.set_param("/ronex_" + ronex_id[0] + "_sensor_controller/type",
                                "sr_ronex_controllers/SPISensorReadController")
                rospy.set_param("/ronex_" + ronex_id[0] + "_sensor_controller/ronex_id", ronex_id[0])

    def load_and_start_ctrl(self, ronex_ids):
        """
        Load and start the sensor controllers for the list of given ronexes.
        ...
        controllers_list = []
        for ronex_id in ronex_ids:
            controllers_list.append("ronex_" + ronex_id[0] + "_sensor_controller")
...
if __name__ == "__main__":
    rospy.sleep(2.5)
    rospy.init_node("load_sensor_controller")
    load_passthrough = LoadSensorControllers()
