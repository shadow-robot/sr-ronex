...
class SpiRobotConnector():
    ...
    def __init__(self):
        ...
        self.waitress = [rospy.ServiceProxy("/ronex/spi/1402573926/command/passthrough/0", SPI),
                         rospy.ServiceProxy("/ronex/spi/1402573926/command/passthrough/1", SPI),
                         rospy.ServiceProxy("/ronex/spi/1402574043/command/passthrough/0", SPI),
                         rospy.ServiceProxy("/ronex/spi/1402574043/command/passthrough/1", SPI),
                         rospy.ServiceProxy("/ronex/spi/1402574043/command/passthrough/2", SPI),
                         rospy.ServiceProxy("/ronex/spi/1402574043/command/passthrough/3", SPI)]
        ...
        self.publishers = [rospy.Publisher("/sh_{}_position_controller/command".format(j), Float64, latch=True)
    ...
    def get_spi_values(self):
        ...
        bin_results = ('{0:08b}{1:08b}'.format(int(ch[1]), int(ch[2])) for res in responses for ch in res)
        ...
        # return [0,1,2,3]
        # pots   [L,R,M,F]
        results = [int(res[4:], 2) for res in bin_results]
        print results
        return [0.000523599*(min(max(res, 600), 3300) - 600) for res in results]
