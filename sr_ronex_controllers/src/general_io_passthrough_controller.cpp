...
bool GeneralIOPassthroughController::init(ros_ethercat_model::RobotStateInterface* robot, ros::NodeHandle &n)
{
  ...
  for ( size_t i = 0; i < general_io_->command_.digital_.size(); ++i)
  {
    ...
    digital_subscribers_.push_back(
            node_.subscribe<std_msgs::Bool>(sub_topic.str(), 1,
                                            boost::bind(&GeneralIOPassthroughController::digital_commands_cb,
                                                        this, _1,  i)));
  }
  ...
  for ( size_t i = 0; i < general_io_->command_.pwm_.size(); ++i)
  {
    ...
    pwm_subscribers_.push_back(
            node_.subscribe<sr_ronex_msgs::PWM>(sub_topic.str(), 1,
                                                boost::bind(&GeneralIOPassthroughController::pwm_commands_cb,
                                                            this, _1, i)));
  }
