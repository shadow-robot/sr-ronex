...
namespace sr_cod_decod
{
  ...
  void CodDecodStdIo::construct(hardware_interface::HardwareInterface *hw, EtherCAT_SlaveHandler *sh,
                                int n_digital_outputs, int n_analog_outputs, int n_digital_inputs, int n_analog_inputs,
                                int n_PWM_outputs)
  {
    ...
    snprintf(buff, 200, "device_0x%08X_0x%08X_digital_outputs_command",
            ...
            sh_->get_serial());
    topic = buff;
    sub_digital_output_command_ = node_.subscribe<sr_ronex_msgs::BoolArray>
    ...
    snprintf(buff, 200, "device_0x%08X_0x%08X_PWM_outputs_command",
            ...
            sh_->get_serial());
        topic = buff;
    sub_PWM_output_command_ = node_.subscribe<std_msgs::UInt16MultiArray>
    ...
    snprintf(buff, 200, "device_0x%08X_0x%08X_digital_inputs_state",
            ...
            sh_->get_serial());
    topic = buff;
    digital_input_state_publisher_.reset(
    ...
    snprintf(buff, 200, "device_0x%08X_0x%08X_analog_inputs_state",
            ...
            sh_->get_serial());
    topic = buff;
    analog_input_state_publisher_.reset(
            new realtime_tools::RealtimePublisher<std_msgs::UInt16MultiArray>(node_, topic, 1));
  }
  ...
  void CodDecodStdIo::build_command(unsigned char *command_buffer)
  {
    ...
    for (size_t i = 0; i < d_out_ptr->data.size(); i++)
    {
      if (d_out_ptr->data.at(i))
        (*(reinterpret_cast<uint8_t *>(buff_ptr))) |= (0x01 << (i % 8));
      if ((i + 1) % 8 == 0)
        ++buff_ptr;
    }
    ...
    {
      *(reinterpret_cast<uint16_t *>(buff_ptr)) = PWM_out_ptr->data.at(i);
      buff_ptr += 2;
    }
    ...
    {
      *(reinterpret_cast<uint16_t *>(buff_ptr)) = a_out_ptr->data.at(i);
      buff_ptr += 2;
    }
  ...
  void CodDecodStdIo::PWMOutputCommandCB(const std_msgs::UInt16MultiArrayConstPtr& msg)
  {
    ...
    {
      ROS_ERROR("Wrong number of PWM outputs. Must be: %d. Remember that you need 2 UINT values for every output "
                        "(Period, ON-time)", n_PWM_outputs_);
    }
  }
  ...
  {
    // Read the digital outputs from the digital_output_ realtime box and set the right values to the
    // digital_output_ realtime box again
    boost::shared_ptr<sr_ronex_msgs::BoolArray> d_out_ptr(new sr_ronex_msgs::BoolArray());
  ...
  {
    // The following line connects the digital input input_pin to the variable pointed by where_to_store_it
    // (it should point to a field in the HW interface)
  ...
  {
    // The following line connects the analog input input_pin to the variable pointed by where_to_store_it
    // (it should point to a field in the HW interface)
  ...
  {
    // Read the PWM outputs from the PWM_output_ realtime box and set the right values to the
    // PWM_output_ realtime box again
  ...
  void CodDecodStdIo::doubleToPWMOutput(sr_cod_decod_std_io::DigitalIo output_pin, uint16_t PWM_period,
                                        double PWM_ON_time)
  {
    uint16ToPWMOutput(output_pin, PWM_period, static_cast<uint16_t>
    (abs(PWM_ON_time) > static_cast<double>(PWM_period + 1) ? static_cast<double>(PWM_period + 1) : abs(PWM_ON_time) +
                    0.5));
  }

  void CodDecodStdIo::uint16ToPWMOutput(
          sr_cod_decod_std_io::DigitalIo output_pin, uint16_t PWM_period, uint16_t PWM_ON_time)
  {
    // Read the PWM outputs from the PWM_output_ realtime box and set the right values to the PWM_output_ realtime
    // box again
    boost::shared_ptr<std_msgs::UInt16MultiArray> PWM_out_ptr(new std_msgs::UInt16MultiArray());
    ...
    PWM_out_ptr->data.at(output_pin * 2 + 1) = static_cast<uint16_t>(PWM_ON_time > (PWM_period + 1) ? (PWM_period + 1) :
                                                                     PWM_ON_time);
    PWM_output_.set(PWM_out_ptr);
  }
