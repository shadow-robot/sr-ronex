...
namespace sr_cod_decod
{
  ...
  CodDecodManager::CodDecodManager(EtherCAT_SlaveHandler *sh, int n_digital_outputs, int n_analog_outputs, int n_digital_inputs, int n_analog_inputs, int n_PWM_outputs)
    :cod_decod_loader_("sr_ronex_drivers", "sr_cod_decod::CodDecod")
  {
    ...
    if (cod_decod_ != NULL)
    {
      cod_decod_->construct(sh, n_digital_outputs, n_analog_outputs, n_digital_inputs, n_analog_inputs, n_PWM_outputs);
    }
